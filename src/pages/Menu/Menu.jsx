...
const _itemContains = (item, filter) => {
  if (!filter) {
    return true;
  }

  const lowerFilter = filter.toLowerCase();
  return item.name.toLowerCase().includes(lowerFilter) || item.category?.toLowerCase().includes(lowerFilter);
}
...
const Menu = ({recipes}) => {
  ...
  const sortedRecipes = recipes
    .filter(item => _itemContains(item, filter))
